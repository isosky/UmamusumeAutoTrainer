...
expect_attribute_mubailaien = [1100, 800, 600, 250, 300]
...
for i in range(1):
    for k, v in attachment_data_mubailaien.items():
        log.info(f"{k}:{v}")
    task = app_config.build_task(
        task_execute_mode, task_type, task_desc, cron_job_config, attachment_data_mubailaien)
